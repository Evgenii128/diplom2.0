// admin.js - функции для панели управления
...
async function loadUserInfo() {
    try {
        ...
        if (data.success && data.user) {
            document.getElementById('adminUserName').textContent = data.user.name || data.user.username;
...
function toggleTheme() {
    ...
    showNotification(`Переключена ${newTheme === 'dark' ? 'тёмная' : 'светлая'} тема`, 'info');
}
...
function loadSectionData(sectionId) {
    switch(sectionId) {
        ...
        case 'price-lists': loadPriceLists(); break;
...
async function loadRequests() {
    try {
        ...
        tableBody.innerHTML = '<tr><td colspan="7" class="loading-state"><i class="fas fa-spinner fa-spin"></i> Загрузка заявок...</td></tr>';
        ...
        requests.forEach(req => {
            ...
            switch(req.status) {
                case 'new': statusClass = 'pending'; statusText = 'Новая'; break;
                case 'in_progress': statusClass = 'active'; statusText = 'В работе'; break;
                case 'completed': statusClass = 'paid'; statusText = 'Завершена'; break;
                case 'cancelled': statusClass = 'cancelled'; statusText = 'Отклонена'; break;
            }
...
async function createContractFromRequest() {
    ...
    try {
        ...
        if (request.email) {
            const clientsResponse = await fetch(`${API_BASE_URL}/clients?search=${encodeURIComponent(request.email)}`);
        ...
        if (!clientId) {
            ...
            if (!clientResult.success) {
                throw new Error('Ошибка создания клиента: ' + (clientResult.error || 'Неизвестная ошибка'));
            }
...
async function createInvoiceFromContract(contractId, amount) {
    try {
        ...
        const contractResponse = await fetch(`${API_BASE_URL}/contracts/${contractId}`);
...
async function viewRequestDetails(id) {
    try {
        ...
        try {
            ...
            servicesHtml = selectedServices.map(s => {
...
async function loadDashboardData() {
    try {
        ...
        document.getElementById('statNewRequests').textContent = data.newRequests || 0;
        ...
        if (document.getElementById('statActiveContracts')) {
            document.getElementById('statActiveContracts').textContent = data.activeContracts || 0;
        }
        if (document.getElementById('statPendingInvoices')) {
            document.getElementById('statPendingInvoices').textContent = data.pendingInvoices || 0;
        }
...
async function loadServices() {
    try {
        ...
        tableBody.innerHTML = '<tr><td colspan="6" class="loading-state"><i class="fas fa-spinner fa-spin"></i> Загрузка...</td></tr>';
...
async function saveService(event) {
    ...
    try {
        ...
        if (response.ok && result.success) {
            ...
            loadServices();
            loadDashboardData();
            showNotification(serviceId ? 'Услуга обновлена' : 'Услуга добавлена', 'success');
            document.getElementById('serviceForm').reset();
            document.getElementById('serviceId').value = '';
            document.getElementById('modalServiceTitle').innerHTML = '<i class="fas fa-concierge-bell"></i> Добавить новую услугу';
...
async function editService(id) {
    try {
        ...
        document.getElementById('modalServiceTitle').innerHTML = '<i class="fas fa-edit"></i> Редактировать услугу';
...
async function saveClient(event) {
    ...
    try {
        ...
        if (response.ok && result.success) {
            ...
            loadClients();
            loadDashboardData();
            showNotification(clientId ? 'Клиент обновлен' : 'Клиент добавлен', 'success');
            document.getElementById('clientForm').reset();
            document.getElementById('clientId').value = '';
            document.getElementById('modalClientTitle').innerHTML = '<i class="fas fa-user-plus"></i> Добавить нового клиента';
        } else {
            throw new Error(result.error || 'Ошибка сохранения');
        }
    } catch (error) {
...
async function editClient(id) {
    try {
        ...
        document.getElementById('modalClientTitle').innerHTML = '<i class="fas fa-edit"></i> Редактировать клиента';
...
async function loadPriceLists() {
    try {
        ...
        container.innerHTML = '<div class="loading-state"><i class="fas fa-spinner fa-spin"></i> Загрузка прайс-листов...</div>';
        ...
        if (priceLists.length === 0) {
            container.innerHTML = '<div class="empty-state"><i class="fas fa-file-invoice-dollar"></i><p>Прайс-листы не найдены</p><button class="btn btn-primary mt-2" onclick="showModal(\'priceListModal\')">Создать первый</button></div>';
            return;
        }
...
async function savePriceList(event) {
    ...
    try {
        ...
        const url = priceListId ? `${API_BASE_URL}/price-lists/${priceListId}` : `${API_BASE_URL}/price-lists`;
        ...
        if (response.ok && result.success) {
            ...
            const items = [];
            document.querySelectorAll('#priceListItems .price-list-item').forEach(item => {
                const serviceSelect = item.querySelector('.item-service');
                const priceInput = item.querySelector('.item-price');
                
                if (serviceSelect && serviceSelect.value && priceInput && priceInput.value) {
                    items.push({
            ...
            if (items.length > 0) {
                await fetch(`${API_BASE_URL}/price-list-items/${newPriceListId || priceListId}`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
            ...
            loadPriceLists();
            showNotification(result.message || (priceListId ? 'Прайс-лист обновлен' : 'Прайс-лист создан'), 'success');
            ...
            document.getElementById('modalPriceListTitle').innerHTML = '<i class="fas fa-file-invoice-dollar"></i> Создать прайс-лист';
...
async function editPriceList(id) {
    try {
        ...
        document.getElementById('priceListName').value = priceList.price_list_name || '';
        document.getElementById('priceListDescription').value = priceList.description || '';
        document.getElementById('priceListValidFrom').value = priceList.valid_from;
        document.getElementById('priceListValidTo').value = priceList.valid_to || '';
        ...
        document.getElementById('modalPriceListTitle').innerHTML = '<i class="fas fa-edit"></i> Редактировать прайс-лист';
...
async function deletePriceList(id) {
    askDelete(async () => {
        try {
            const response = await fetch(`${API_BASE_URL}/price-lists/${id}`, { method: 'DELETE' });
            const result = await response.json();
            if (result.success) {
...
async function loadServicesForSelect(selectElement, selectedServiceId = null) {
    try {
        ...
        services.forEach(s => {
            const selected = selectedServiceId && s.id == selectedServiceId ? 'selected' : '';
        ...
        selectElement.addEventListener('change', function() {
            ...
            const priceInput = this.closest('div').parentElement.querySelector('.item-price');
        ...
        if (selectedServiceId) {
            const selectedOption = selectElement.querySelector(`option[value="${selectedServiceId}"]`);
            if (selectedOption) {
                const priceInput = selectElement.closest('div').parentElement.querySelector('.item-price');
...
async function loadContracts() {
    try {
        ...
        tableBody.innerHTML = '<tr><td colspan="7" class="loading-state"><i class="fas fa-spinner fa-spin"></i> Загрузка...</td></tr>';
        ...
        contracts.forEach(contract => {
            const endDate = contract.end_date ? formatDate(contract.end_date) : 'Бессрочный';
            let statusText = contract.status === 'active' ? 'Активен' : (contract.status === 'suspended' ? 'Приостановлен' : 'Завершен');
            let statusClass = contract.status === 'active' ? 'active' : (contract.status === 'suspended' ? 'pending' : 'cancelled');
...
async function saveContract(event) {
    ...
    const contractData = {
        ...
        price_list_id: document.getElementById('contractPriceList').value ? parseInt(document.getElementById('contractPriceList').value) : null,
        ...
        total_amount: parseFloat(document.getElementById('contractAmount').value) || 0,
    ...
    try {
        ...
        if (response.ok && result.success) {
            ...
            loadContracts();
            loadDashboardData();
            showNotification(contractId ? 'Договор обновлен' : 'Договор создан', 'success');
            document.getElementById('contractForm').reset();
            document.getElementById('contractId').value = '';
            document.getElementById('modalContractTitle').innerHTML = '<i class="fas fa-file-contract"></i> Добавить договор';
        } else {
...
async function editContract(id) {
    try {
        ...
        document.getElementById('contractAmount').value = contract.total_amount || 0;
        ...
        setTimeout(() => {
            ...
            if (contract.price_list_id) {
                document.getElementById('contractPriceList').value = contract.price_list_id;
            }
        }, 100);
        
        document.getElementById('modalContractTitle').innerHTML = '<i class="fas fa-edit"></i> Редактировать договор';
...
async function deleteContract(id) {
    askDelete(async () => {
        try {
            const response = await fetch(`${API_BASE_URL}/contracts/${id}`, { method: 'DELETE' });
            const result = await response.json();
            if (result.success) {
...
async function viewContract(id) {
    try {
        ...
        const clientResponse = await fetch(`${API_BASE_URL}/clients/${contract.client_id}`);
        const clientResult = await clientResponse.json();
...
async function createInvoiceFromContractDirect(contractId) {
    try {
        ...
        document.getElementById('invoiceAmount').value = contract.data.total_amount || 0;
        ...
        document.getElementById('invoiceNumber').value = `СЧ-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 1000)).padStart(3, '0')}`;
