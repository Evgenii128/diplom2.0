...
async function initializeDatabase() {
    ...
    try {
        ...
        const [usersCount] = await connection.execute('SELECT COUNT(*) as count FROM users WHERE username = ?', ['admin']);
        if (usersCount[0].count === 0) {
            const hashedPassword = await bcrypt.hash('admin123', 10);
            await connection.execute(
                `INSERT INTO users (username, email, password_hash, role, first_name, last_name, is_active) 
                 VALUES (?, ?, ?, ?, ?, ?, ?)`,
                ['admin', 'admin@rzd-tehservice.ru', hashedPassword, 'admin', 'Администратор', 'Системы', true]
            );
...
app.post('/api/admin/login', async (req, res) => {
    ...
    try {
        ...
        await logAudit('LOGIN', 'users', `Вход пользователя: ${user.username}`, user.username);
...
// ПУБЛИЧНЫЙ РОУТ ДЛЯ ЗАЯВОК
app.post('/api/public/requests', async (req, res) => {
    const connection = await pool.getConnection();
    try {
        console.log('📥 Получены данные заявки:', JSON.stringify(req.body, null, 2));
        ...
        if (!selected_services || !Array.isArray(selected_services) || selected_services.length === 0) {
            return res.status(400).json({ success: false, error: 'Выберите хотя бы одну услугу' });
        }

        const serviceDescription = selected_services.map(s => 
            `${s.service_name} (x${s.quantity}) - ${Number(s.subtotal).toFixed(2)} ₽`
        ).join('; ');

        const [rows] = await connection.execute("SELECT COUNT(*) as count FROM requests");
        const requestNumber = `REQ-${new Date().getFullYear()}-${String(rows[0].count + 1).padStart(3, '0')}`;
        ...
        console.log('✅ Заявка сохранена, ID:', result.insertId);

        try {
            await connection.execute(
                `INSERT INTO audit_logs (username, action, table_name, details) 
                 VALUES (?, ?, ?, ?)`,
                ['Клиент', 'INSERT', 'requests', `Новая заявка ${requestNumber} на сумму ${total_amount} ₽`]
            );
        } catch (logError) {
...
app.get('/api/requests/:id', async (req, res) => {
    try {
        ...
        if (request.selected_services && typeof request.selected_services === 'string') {
            try {
...
app.put('/api/requests/:id/status', async (req, res) => {
    try {
        ...
        if (!status) {
            return res.status(400).json({ success: false, error: 'Статус не указан' });
        }
        ...
        await logAudit('UPDATE', 'requests', `Статус заявки ID:${id} изменен на ${status}`, 'Администратор');
...
app.post('/api/services', async (req, res) => {
    try {
        let { service_code, service_name, description, unit, base_price, is_active } = req.body;
        

        description = description !== undefined ? description : null;
        is_active = is_active !== undefined ? is_active : true;
        
        const [existing] = await pool.execute('SELECT id FROM services WHERE service_code = ?', [service_code]);
        if (existing.length > 0) {
            return res.status(400).json({ success: false, error: `Код услуги '${service_code}' уже существует` });
        }
        ...
        await logAudit('INSERT', 'services', `Создана услуга: ${service_code} - ${service_name}`, 'Администратор');
        
        res.status(201).json({
            success: true,
            message: 'Услуга успешно добавлена',
            data: { id: result.insertId, service_code, service_name, description, unit, base_price, is_active }
        });
    } catch (error) {
...
app.put('/api/services/:id', async (req, res) => {
    try {
        ...
        if (existing.length > 0) {
            return res.status(400).json({ success: false, error: `Код услуги '${service_code}' уже используется` });
        }
        ...
        const [result] = await pool.execute(
            `UPDATE services 
             ...
             WHERE id = ?`,
            [service_code, service_name, description, unit, base_price, is_active, id]
        );
        ...
        await logAudit('UPDATE', 'services', `Обновлена услуга ID: ${id}`, 'Администратор');
...
app.delete('/api/services/:id', async (req, res) => {
    try {
        ...
        const [items] = await pool.execute('SELECT id FROM price_list_items WHERE service_id = ?', [id]);
        if (items.length > 0) {
            return res.status(400).json({ 
                success: false, 
        ...
        await logAudit('DELETE', 'services', `Удалена услуга ID: ${id}`, 'Администратор');
...
app.post('/api/clients', async (req, res) => {
    try {
        let { company_name, contact_person, tax_number, phone, email, legal_address, bank_details } = req.body;
        ...
        const [result] = await pool.execute(
            `INSERT INTO clients (company_name, contact_person, tax_number, phone, email, legal_address, bank_details) 
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [company_name, contact_person, tax_number, phone, email, legal_address, bank_details]
        );
        
        await logAudit('INSERT', 'clients', `Создан клиент: ${company_name || 'Без названия'}`, 'Администратор');
        
        res.status(201).json({
            success: true,
            message: 'Клиент успешно добавлен',
            data: { id: result.insertId, company_name, contact_person, tax_number, phone, email, legal_address, bank_details }
        });
    } catch (error) {
    ...
    try {
        ...
        const [result] = await pool.execute(
            `UPDATE clients 
             ...
             WHERE id = ?`,
            [company_name, contact_person, tax_number, phone, email, legal_address, bank_details, id]
        );
        ...
        await logAudit('UPDATE', 'clients', `Обновлен клиент ID: ${id}`, 'Администратор');
...
app.delete('/api/clients/:id', async (req, res) => {
    try {
        ...
        const [contracts] = await pool.execute('SELECT id FROM contracts WHERE client_id = ?', [id]);
        if (contracts.length > 0) {
            return res.status(400).json({ 
                success: false, 
                error: 'Нельзя удалить клиента, у которого есть договоры. Сначала удалите договоры.' 
            });
        }
        ...
        await logAudit('DELETE', 'clients', `Удален клиент ID: ${id}`, 'Администратор');
...
app.get('/api/price-lists', async (req, res) => {
    try {
        const [rows] = await pool.execute('SELECT * FROM price_lists ORDER BY valid_from DESC');
        res.json(rows);
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
...
app.get('/api/price-lists/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await pool.execute('SELECT * FROM price_lists WHERE id = ?', [id]);
        
        if (rows.length === 0) {
            return res.status(404).json({ success: false, message: 'Прайс-лист не найден' });
        }
...
app.post('/api/price-lists', async (req, res) => {
    try {
        let { price_list_name, description, valid_from, valid_to, is_active } = req.body;
        ...
        await logAudit('INSERT', 'price_lists', `Создан прайс-лист: ${price_list_name}`, 'Администратор');
        
        res.status(201).json({
            success: true,
            message: 'Прайс-лист успешно создан',
            data: { id: result.insertId, price_list_name, description, valid_from, valid_to, is_active }
        });
    } catch (error) {
...
app.put('/api/price-lists/:id', async (req, res) => {
    try {
        ...
        await logAudit('UPDATE', 'price_lists', `Обновлен прайс-лист ID: ${id}`, 'Администратор');
...
app.delete('/api/price-lists/:id', async (req, res) => {
    try {
        ...
        const [contracts] = await pool.execute('SELECT id FROM contracts WHERE price_list_id = ?', [id]);
        if (contracts.length > 0) {
            return res.status(400).json({ 
                success: false, 
        ...
        const [result] = await pool.execute('DELETE FROM price_lists WHERE id = ?', [id]);
        
        if (result.affectedRows === 0) {
            return res.status(404).json({ success: false, message: 'Прайс-лист не найден' });
        }
        
        await logAudit('DELETE', 'price_lists', `Удален прайс-лист ID: ${id}`, 'Администратор');
...
app.post('/api/price-list-items/:priceListId', async (req, res) => {
    ...
    try {
        ...
        await connection.execute('DELETE FROM price_list_items WHERE price_list_id = ?', [priceListId]);
        ...
        await logAudit('UPDATE', 'price_list_items', `Обновлены позиции прайс-листа ID: ${priceListId}`, 'Администратор');
...
app.get('/api/contracts', async (req, res) => {
    try {
        const { status } = req.query;
        let sql = 'SELECT c.*, cl.company_name, cl.contact_person FROM contracts c LEFT JOIN clients cl ON c.client_id = cl.id WHERE 1=1';
        const params = [];
...
app.post('/api/contracts', async (req, res) => {
    try {
        let { contract_number, client_id, price_list_id, contract_date, start_date, end_date, total_amount, status } = req.body;
        ...
        const [existing] = await pool.execute('SELECT id FROM contracts WHERE contract_number = ?', [contract_number]);
        if (existing.length > 0) {
            return res.status(400).json({ success: false, error: `Договор с номером '${contract_number}' уже существует` });
        }
        
        const [result] = await pool.execute(
            `INSERT INTO contracts (contract_number, client_id, price_list_id, contract_date, start_date, end_date, total_amount, status) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [contract_number, client_id, price_list_id, contract_date, start_date, end_date, total_amount, status]
        );
        
        await logAudit('INSERT', 'contracts', `Создан договор: ${contract_number}`, 'Администратор');
        
        res.status(201).json({
            success: true,
            message: 'Договор успешно создан',
            data: { id: result.insertId, contract_number, client_id, price_list_id, contract_date, start_date, end_date, total_amount, status }
        });
    } catch (error) {
    ...
    try {
        ...
        const [result] = await pool.execute(
            `UPDATE contracts 
             ...
             WHERE id = ?`,
            [contract_number, client_id, price_list_id, contract_date, start_date, end_date, total_amount, status, id]
        );
        ...
        await logAudit('UPDATE', 'contracts', `Обновлен договор ID: ${id}`, 'Администратор');
...
app.delete('/api/contracts/:id', async (req, res) => {
    try {
        ...
        const [invoices] = await pool.execute('SELECT id FROM invoices WHERE contract_id = ?', [id]);
        if (invoices.length > 0) {
            return res.status(400).json({ 
                success: false, 
                error: 'Нельзя удалить договор, у которого есть счета. Сначала удалите счета.' 
            });
        }
        ...
        await logAudit('DELETE', 'contracts', `Удален договор ID: ${id}`, 'Администратор');
...
app.get('/api/invoices', async (req, res) => {
    try {
        const { status } = req.query;
        let sql = 'SELECT i.*, c.contract_number, cl.company_name FROM invoices i LEFT JOIN contracts c ON i.contract_id = c.id LEFT JOIN clients cl ON c.client_id = cl.id WHERE 1=1';
        const params = [];
...
app.post('/api/invoices', async (req, res) => {
    try {
        let { invoice_number, contract_id, invoice_date, due_date, amount, tax_amount, total_amount, status, notes } = req.body;
        ...
        const [existing] = await pool.execute('SELECT id FROM invoices WHERE invoice_number = ?', [invoice_number]);
        if (existing.length > 0) {
            return res.status(400).json({ success: false, error: `Счет с номером '${invoice_number}' уже существует` });
        }
        
        const [result] = await pool.execute(
            `INSERT INTO invoices (invoice_number, contract_id, invoice_date, due_date, amount, tax_amount, total_amount, status, notes) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [invoice_number, contract_id, invoice_date, due_date, amount, tax_amount, total_amount, status, notes]
        );
        
        await logAudit('INSERT', 'invoices', `Создан счет: ${invoice_number}`, 'Администратор');
        
        res.status(201).json({
            success: true,
            message: 'Счет успешно создан',
            data: { id: result.insertId, invoice_number, contract_id, invoice_date, due_date, amount, tax_amount, total_amount, status, notes }
        });
    } catch (error) {
...
app.put('/api/invoices/:id', async (req, res) => {
    try {
        const { id } = req.params;
        let { invoice_number, contract_id, invoice_date, due_date, amount, tax_amount, total_amount, status, notes, payment_date } = req.body;
        ...
        const [result] = await pool.execute(
            `UPDATE invoices 
             ...
             WHERE id = ?`,
            [invoice_number, contract_id, invoice_date, due_date, amount, tax_amount, total_amount, status, notes, payment_date, id]
        );
        ...
        await logAudit('UPDATE', 'invoices', `Обновлен счет ID: ${id}`, 'Администратор');
...
app.put('/api/invoices/:id/pay', async (req, res) => {
    try {
        ...
        if (result.affectedRows === 0) {
            return res.status(404).json({ success: false, message: 'Счет не найден или уже оплачен' });
        }
        
        await logAudit('UPDATE', 'invoices', `Счет ID: ${id} отмечен как оплаченный`, 'Администратор');
...
app.delete('/api/invoices/:id', async (req, res) => {
    try {
        ...
        await logAudit('DELETE', 'invoices', `Удален счет ID: ${id}`, 'Администратор');
...
app.get('/api/dashboard/stats', async (req, res) => {
    try {
        const [reqNew] = await pool.execute("SELECT COUNT(*) as count FROM requests WHERE status = 'new'");
        const [reqTotal] = await pool.execute("SELECT COUNT(*) as count FROM requests");
        const [clients] = await pool.execute("SELECT COUNT(*) as count FROM clients");
        const [services] = await pool.execute("SELECT COUNT(*) as count FROM services WHERE is_active = true");
        const [contracts] = await pool.execute("SELECT COUNT(*) as count FROM contracts WHERE status = 'active'");
        const [invoices] = await pool.execute("SELECT COUNT(*) as count FROM invoices WHERE status = 'pending'");
        const [totalRevenue] = await pool.execute("SELECT SUM(total_amount) as total FROM invoices WHERE status = 'paid'");
...
app.get('/api/health', async (req, res) => {
    try {
        ...
        connection.release();
        res.json({ status: 'OK', database: { connected: true, version: rows[0].version } });
    } catch (error) {
...
async function startServer() {
    ...
    app.listen(PORT, () => {
        ...
        console.log('='.repeat(60));
        console.log(`📊 API статистика:    http://localhost:${PORT}/api/dashboard/stats`);
        ...
        console.log(`📋 API прайс-листы:   http://localhost:${PORT}/api/price-lists`);
        console.log('='.repeat(60));
